...
function App() {
  ...
  useEffect(() => {
    Papa.parse(filePath(subject), {
      ...
      complete: (results) => {
        ...
        const classesSetx = new Set(parsedData.map(student => student.class));

        const x = [];
        for (const className of classesSetx) {
  ...
  useEffect(() => {
    if (classFilter) {
      ...
      // Reset to all students if no filter is applied
  ...
  return (
      <div style={{padding: '20px'}}>
        ...
        <div>
         Môn: <select value={subject} onChange={(e) => setSubject(e.target.value)}>
        ...
        <select value={classFilter} onChange={(e) => setClassFilter(e.target.value)}>
          ...
          {(classes || []).map((obj, index) => {
            return <option key={index} value={obj.name}>{obj.name} ({obj.students.length} students)</option>;
          })
          }
        </select>
            {' '} <span style={{marginLeft: '10px'}}>Đang chọn: </span><strong style={{color: 'green'}}>
          {classFilter}
        </strong>

          <input value={filteredStudent}
        ...
        <h2>Có <span style={{color:'blue'}}>{allStudents.length}</span> hs |
          <span style={{color:'blue'}}>{classes.length}</span>{' '}
          lớp</h2>

        {classFilter ? null :
        <div>
          <h2>So sánh ĐTB giữa các lớp</h2>
          <ScoreSpectrumChart students={[...classes].map((g) => {
            return {
        ...
        <h2>Danh sách học sinh
          <span  onClick={() => {setAscending(!ascending);}}
